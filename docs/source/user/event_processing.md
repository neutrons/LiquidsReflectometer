...
assigned a weight $w$ to account for various corrections. Summing events then becomes the
...
$$
C_{par} = -\Re \left( W_0(-R\tau/\Delta_{TOF})  \right) \Delta_{TOF}/R
$$

where $R$ is the number of triggers per accelerator pulse within a time-of-flight bin $\Delta_{TOF}$.
The dead time for the current BL4B detector is $\tau=4.2 \mu s$. In the equation above, $\Re(W_0)$ refers to the principal branch of the Lambert W function.
...
and replacing $\theta$ by the value at which the reflected beam was measured. We are then
effectively computing what the measured signal would be if all neutron from the beam would reflect
with a probability of 1. We refer this distribution at $S_1(q_z)$.
...
With this new weight, we can compute reflectivity directly from the $S(q_z)$ equation above:
