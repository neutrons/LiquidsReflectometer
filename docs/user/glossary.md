...
- **run (partial)**: A run (also called a partial) is an individual measurement or data collection event within a sequence. Each run has its own unique identifier and may have specific settings or parameters. Partial data files are typically named `REFL_{sequence_id}_{sequence_number}_{run_number}_partial.ort`
