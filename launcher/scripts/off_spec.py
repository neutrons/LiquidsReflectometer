...
class PixelData:
    ...
    def __init__(self, run_number):
        self.ws = api.LoadEventNexus(
            "REF_L_%s" % run_number, OutputWorkspace="r%s" % run_number
        )
    ...
    def get_parameters(self):
        if self.ws.getInstrument().hasParameter("sample-det-distance"):
            self.det_distance = self.ws.getInstrument().getNumberParameter(
                "sample-det-distance"
            )[0]

        if self.ws.getInstrument().hasParameter("source-det-distance"):
            self.source_detector_distance = self.ws.getInstrument().getNumberParameter(
                "source-det-distance"
            )[0]

        # Set up basic data
        self.n_x = int(
            self.ws.getInstrument().getNumberParameter("number-of-x-pixels")[0]
        )
        self.n_y = int(
            self.ws.getInstrument().getNumberParameter("number-of-y-pixels")[0]
        )

        self.pixel_width = (
            float(self.ws.getInstrument().getNumberParameter("pixel-width")[0]) / 1000.0
        )
    ...
    def process(self, output_dir, wavelength_step=1):
        ...
        _ws = api.Rebin(
            InputWorkspace=self.ws, Params="%s,%s,%s" % (tof_min, tof_step, tof_max)
        )
        y = _ws.extractY()
        x = _ws.extractX()
