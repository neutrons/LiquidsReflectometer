...
def process(run_number, db_run_number, peak_pixel, db_peak_pixel, output_dir=None):
    ...
    qz, r, dr = workflow.reduce_explorer(
        ws, ws_db, center_pixel=peak_pixel, db_center_pixel=db_peak_pixel
    )

    # Save reduced data
    np.savetxt(
        os.path.join(output_dir, "r%s_quick_reduce.txt" % run_number),
        np.asarray([qz, r, dr]).T,
    )
