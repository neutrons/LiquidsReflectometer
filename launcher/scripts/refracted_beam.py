...
class Refracted(object):
    def __init__(self, ws, material="Si", tof_bin=200, offset=0.01, pixel_size=0.00072):
        ...
        _ws = api.Rebin(
            InputWorkspace=ws, Params="%s,%s,%s" % (self.tof_min, tof_bin, self.tof_max)
        )
    ...
    def fit_refracted_pixel(self):
        ...
        for i in range(self.p_vs_t.shape[1]):
            ...
            fit = gauss.fit(
                self.p_vs_t[y_pixel_min:y_pixel_max, i],
                pars,
                method="leastsq",
                x=self.y_pixel[y_pixel_min:y_pixel_max],
            )

            fit.params["g_amplitude"].value
    ...
    def pixel_prediction(self, wl, offset=0.01):
        ...
        return (
            np.sin(np.pi / 180 * (_angle + _theta_specular))
            * self.det_distance
            / self.pixel_size
            + self.center_db
        )
    ...
    def compute_angle_from_pixel(self, pixels, offset, pixel_size=None):
        ...
        return (
            180.0
            / np.pi
            * np.arcsin((pixel_size * (pixels - self.center_db) / self.det_distance))
            - _theta_specular
        )
    ...
    def residuals(self, pars, wl, data, err):
        ...
        _theta_specular = self.theta_sample + offset
        _spec_pixel = (
            np.sin(np.pi / 180 * (2 * _theta_specular)) * self.det_distance / pixel_size
            + self.center_db
        )

        resid = (_refr_pixel - data) ** 2 / err**2 + (
            _spec_pixel - self.center_r
        ) ** 2 / self.d_center_r**2
    ...
    def fit_offset_and_pixel(self, margin=0.5):
        ...
        wl_c = (
            4
            * np.pi
            * np.sin(np.pi / 180 * (self.theta_sample + self.offset))
            / self.qc
            - margin
        )
        ...
        fit = lmfit.minimize(
            ...
            method=engine,
            kws={
                "data": self.refracted_pixel[:index_c],
    ...
    def plot_refraction_map(self, output_dir=None):
        ...
        ax.pcolormesh(
            self.wl_boundaries, _theta_boundaries, np.log(self.p_vs_t), cmap=cmap
        )
        ...
        if output_dir is not None:
            plt.savefig(
                os.path.join(output_dir, "%s-detector-map.png" % self.run_number)
            )
        plt.show()
    ...
    def improve_peaks(self, output_dir=None):
        ...
        fit_r = gauss.fit(
            self.y_counts[x_min_r:x_max_r],
            pars,
            method="leastsq",
            x=self.y_pixel[x_min_r:x_max_r],
        )

        fit_r.params["g_amplitude"]
    ...
    def plot_pixel_vs_tof(self, output_dir=None):
        ...
        ax.pcolormesh(
            self.tof_boundaries,
            np.arange(self.p_vs_t.shape[0] + 1),
            np.log(self.p_vs_t),
            cmap=cmap,
        )
        ...
        ax.scatter(
            self.tof_boundaries,
            np.ones(self.tof_boundaries.shape) * self.center_r,
            zorder=1,
            linewidth=2,
            s=0.2,
        )
        ax.scatter(
            self.tof_boundaries,
            np.ones(self.tof_boundaries.shape) * self.center_db,
            zorder=1,
            linewidth=2,
            s=0.2,
        )
        ...
        if output_dir is not None:
            plt.savefig(
                os.path.join(output_dir, "%s-pixel-vs-tof.png" % self.run_number)
            )
    ...
    def compute_reflectivity(self, output_dir):
        print(
            "Computing reflectivity for offset=%g and pixel size %g"
            % (self.offset, self.pixel_size)
        )
        _theta_specular = self.theta_sample + self.offset
        ...
        _theta = (
            ...
            / np.pi
            * np.arcsin(
                (self.pixel_size * (self.y_pixel - self.center_db) / self.det_distance)
            )
            - _theta_specular
        )
        ...
        def _gauss(x, mu):
            _sigma = 0.0004 / 2.35
            return np.exp(-((x - mu) ** 2) / (2 * _sigma**2)) / np.sqrt(
                2 * np.pi * _sigma**2
            )
        ...
        np.savetxt(
            os.path.join(output_dir, "%s-reflectivity.txt" % self.run_number),
...
def process_run(run_number, output_dir, material="Si"):
    ...
    fitted_pixel_prediction = refr.pixel_prediction_with_size(
        refr.wl_center, offset=_offset, pixel_size=_pixel_size
    )
    ...
    ax.set_ylim(
        [refr.center_db, refr.center_db + 0.7 * (refr.center_r - refr.center_db)]
    )
    plt.legend()
