...
def reduce_with_mantid(ws, data_set, apply_db=False, apply_scaling_factor=False):
    ...
    if apply_scaling_factor:
        ...
        kwargs["IncidentMediumSelected"] = data_set.incident_medium_list[
            data_set.incident_medium_index_selected
        ]
...
def reduce_30Hz_from_ws(
    ...
    _q_idx_60 = np.asarray(
        np.where(
            (data_60Hz[0] > _min_q - _tolerance) & (data_60Hz[0] < _max_q + _tolerance)
        )
    )[0]
    _q_idx_meas30 = np.asarray(
        np.where((r_meas[0] > _min_q - _tolerance) & (r_meas[0] < _max_q + _tolerance))
    )[0]
    _q_idx_ref30 = np.asarray(
        np.where((r_ref[0] > _min_q - _tolerance) & (r_ref[0] < _max_q + _tolerance))
    )[0]
    ...
    r_q_final = (
        r_meas[1][_q_idx_meas30] / r_ref[1][_q_idx_ref30] * data_60Hz[1][_q_idx_60]
    )

    dr_q_final = np.sqrt(
        (r_meas[2][_q_idx_meas30] / r_ref[1][_q_idx_ref30] * data_60Hz[1][_q_idx_60])
        ** 2
        + (r_meas[1][_q_idx_meas30] / r_ref[1][_q_idx_ref30] * data_60Hz[2][_q_idx_60])
        ** 2
        + (
            r_meas[1][_q_idx_meas30]
            / r_ref[1][_q_idx_ref30] ** 2
            * data_60Hz[1][_q_idx_60]
            * r_ref[2][_q_idx_ref30]
        )
        ** 2
    )
...
def reduce_60Hz_slices(
    meas_run, template_file, time_interval, output_dir, scan_index=1, create_plot=True
):
    meas_ws = api.LoadEventNexus("REF_L_%s" % meas_run)

    return reduce_60Hz_slices_ws(
        meas_ws,
        template_file,
        time_interval,
        output_dir,
...
def reduce_30Hz_slices_ws(
    ...
    splitws, infows = api.GenerateEventsFilter(
        InputWorkspace=meas_ws_30Hz, TimeInterval=time_interval
    )
...
def reduce_60Hz_slices_ws(
    meas_ws, template_file, time_interval, output_dir, scan_index=1, create_plot=True
):
    """
    Perform 30Hz reduction
    ...
    print("Slicing data")
    splitws, infows = api.GenerateEventsFilter(
        InputWorkspace=meas_ws, TimeInterval=time_interval
    )
    ...
    for name in wsnames:
        ...
        print("workspace %s has %d events" % (name, tmpws.getNumberEvents()))
        try:
            _reduced = reduce_with_mantid(
                tmpws, template_data, apply_scaling_factor=True
            )
            reduced.append(_reduced)
...
if __name__ == "__main__":
    ...
    dynanic30_parser = subparsers.add_parser(
        "dynamic30Hz", help="Reduce time-resolved 30Hz [-h for help]"
    )
    dynanic30_parser.add_argument(
        "meas_run_30Hz", type=int, help="Run number for the data to be processed"
    )
    dynanic30_parser.add_argument(
        "ref_run_30Hz",
        type=str,
    ...
    dynanic30_parser.add_argument(
        "ref_data_60Hz", type=str, help="Reference R(Q), measured at 60Hz"
    )
    dynanic30_parser.add_argument(
        "template_30Hz", type=str, help="File path for the 30Hz reduction template"
    )
    dynanic30_parser.add_argument(
        "time_interval", type=float, help="Time interval to use, in seconds"
    )
    ...
    dynanic60_parser = subparsers.add_parser(
        "dynamic60Hz", help="Reduce time-resolved 60Hz [-h for help]"
    )
    dynanic60_parser.add_argument(
        "meas_run_60Hz", type=int, help="Run number for the data to be processed"
    )
    dynanic60_parser.add_argument(
        "template_60Hz", type=str, help="File path for the 60Hz reduction template"
    )
    dynanic60_parser.add_argument(
        "time_interval", type=float, help="Time interval to use, in seconds"
    )
    dynanic60_parser.add_argument("output_dir", type=str, help="Output directory")
    dynanic60_parser.add_argument(
        "--scan_index",
        type=int,
        dest="scan_index",
