...
if __name__ == "__main__":
    ...
    dynanic_parser.add_argument('run', type=int,
                                help='Run number for the data to be processed')
    dynanic_parser.add_argument('template', type=str,
                                help='File path for the reduction template')
    dynanic_parser.add_argument('time_interval', type=float,
                                help='Time interval to use, in seconds')
    dynanic_parser.add_argument('output_dir', type=str,
                                help='Output directory')
    dynanic_parser.add_argument('--scan_index', type=int, dest='scan_index',
                                help='Template scan index', required=False, default=1)
    dynanic_parser.add_argument('--offset', type=float, dest='theta_offset',
                                help='Theta offset', required=False, default=None)
    ...
    if args.command=='dynamic30Hz':
        ...
        reduced = time_resolved.reduce_30Hz_slices(args.meas_run_30Hz, args.ref_run_30Hz,
                                                   args.ref_data_60Hz, args.template_30Hz,
        ...
        reduced = time_resolved.reduce_slices(args.run, args.template,
                                              time_interval=args.time_interval,
                                              output_dir=args.output_dir,
                                              scan_index=args.scan_index,
