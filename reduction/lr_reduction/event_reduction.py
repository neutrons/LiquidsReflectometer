...
class EventReflectivity(object):
    ...
    def __init__(
        ...
        :param direct_workspace: Mantid workspace containing direct beam data [if None, normalization won't be applied]
    ...
    def extract_meta_data_4B(self):
        ...
        if self._ws_sc.getInstrument().hasParameter("sample-det-distance"):
            self.det_distance = self._ws_sc.getInstrument().getNumberParameter(
                "sample-det-distance"
            )[0]
        else:
        ...
        if self._ws_sc.getInstrument().hasParameter("source-det-distance"):
            self.source_detector_distance = (
                self._ws_sc.getInstrument().getNumberParameter("source-det-distance")[0]
            )
        else:
    ...
    def to_dict(self):
        ...
        sequence_number = int(
            self._ws_sc.getRun().getProperty("sequence_number").value[0]
        )
    ...
    def specular_weighted(self, q_summing=True, bck_in_q=False):
        ...
        if self.signal_bck is not None:
            refl_bck, d_refl_bck = self.bck_subtraction(
                wl_dist=wl_dist, wl_bins=wl_middle, q_summing=bck_in_q
            )
            refl -= refl_bck
            d_refl = np.sqrt(d_refl**2 + d_refl_bck**2)
    ...
    def _bck_subtraction(
        ...
        if bck[0] < peak[0] - 1 and bck[1] < peak[1] + 1:
            ...
            refl_bck, d_refl_bck = self._roi_integration(
                ws,
                peak=_left,
                low_res=low_res,
                q_bins=q_bins,
        ...
        # Background on both sides
            ...
            _refl_bck, _d_refl_bck = self._roi_integration(
                ws,
                peak=_right,
                low_res=low_res,
                q_bins=q_bins,
            ...
            print(
                "Background on both sides: [%s %s] [%s %s]"
                % (_left[0], _left[1], _right[0], _right[1])
            )
    ...
    def _reflectivity(
        ...
        for i in range(low_res[0], int(low_res[1] + 1)):
            for j in range(peak[0], int(peak[1] + 1)):
                ...
                if wl_dist is not None and wl_bins is not None:
                    wl_weights = 1.0 / np.interp(
                        wl_list, wl_bins, wl_dist, np.inf, np.inf
                    )
        ...
        if q_summing:
            ...
            qz_max = (
                ...
                * np.pi
                / self.tof_range[1]
                * self.constant
                * np.fabs(np.sin(theta + delta_theta_f0))
            )
            qz_min = (
                4.0
                * np.pi
                / self.tof_range[1]
                * self.constant
                * np.fabs(np.sin(theta + delta_theta_f1))
            )
        ...
        if wl_dist is not None and wl_bins is not None:
            ...
            non_zero = counts > 0
            d_refl_sq[non_zero] = (
                refl[non_zero] / np.sqrt(counts[non_zero]) / charge / bin_size[non_zero]
            )
    ...
    def off_specular(
        ...
        if self.signal_bck:
            if bck_in_q is None:
                ...
                _, refl_bck, d_refl_bck = self.slice_data(
                    bck_in_q[0],
                    bck_in_q[1],
                    x_bins=qx_bins,
                    z_bins=qz_bins,
    ...
    def gravity_correction(self, ws, wl_list):
        ...
        if ws.getInstrument().hasParameter("s1-sample-distance"):
            s1_sample_distance = (
                ws.getInstrument().getNumberParameter("s1-sample-distance")[0] * 1000
            )
