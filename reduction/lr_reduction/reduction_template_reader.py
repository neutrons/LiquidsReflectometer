...
class ReductionParameters(object):
    ...
    def to_xml(self):
        ...
        _xml += "<data_sets>%s</data_sets>\n" % ",".join(
            [str(i) for i in self.data_files]
        )
        ...
        _xml += "<incident_medium_list>%s</incident_medium_list>\n" % str(
            self.incident_medium_list[0]
        )
        _xml += (
            "<incident_medium_index_selected>%s</incident_medium_index_selected>\n"
    ...
    def from_xml_element(self, instrument_dom):
        ...
        self.data_x_range_flag = getBoolElement(
            instrument_dom, "x_range_flag", default=self.data_x_range_flag
        )

        self.data_x_range = [
            getIntElement(instrument_dom, "x_min_pixel"),
            getIntElement(instrument_dom, "x_max_pixel"),
        ]

        self.norm_x_range_flag = getBoolElement(
            instrument_dom, "norm_x_range_flag", default=self.norm_x_range_flag
        )

        self.norm_x_range = [
            getIntElement(instrument_dom, "norm_x_min"),
            getIntElement(instrument_dom, "norm_x_max"),
        ]

        # background flag
        self.subtract_background = getBoolElement(
            instrument_dom, "background_flag", default=self.subtract_background
        )
        ...
        # TOF range
        self.select_tof_range = getBoolElement(
            instrument_dom, "tof_range_flag", default=self.select_tof_range
        )
        self.tof_range = [
        ...
        self.apply_normalization = getBoolElement(
            instrument_dom, "norm_flag", default=self.apply_normalization
        )
        ...
        self.auto_q_binning = getBoolElement(
            instrument_dom, "auto_q_binning", default=False
        )

        # Angle offset
        self.angle_offset = getFloatElement(
            instrument_dom, "angle_offset", default=self.angle_offset
        )
        self.angle_offset_error = getFloatElement(
            instrument_dom, "angle_offset_error", default=self.angle_offset_error
        )

        # Scaling factor file and options
        self.scaling_factor_file = getStringElement(
        ...
        if getStringList(instrument_dom, "incident_medium_list") != []:
            self.incident_medium_list = getStringList(
                instrument_dom, "incident_medium_list"
            )
            self.incident_medium_index_selected = getIntElement(
...
def getStringList(dom, tag, _default=[]):
    ...
    if len(element_list) > 0:
        for element in element_list:
            elem_list.append(getText(element.childNodes).strip())
    return elem_list
