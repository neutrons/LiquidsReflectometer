...
def reduce_30Hz_from_ws(
    ...
    if template_reference is None:
        r_ref = template.process_from_template_ws(
            ref_ws_30Hz, template_data, q_summing=q_summing, normalize=False
        )
    else:
        r_ref = template.process_from_template_ws(
            ref_ws_30Hz, template_reference, q_summing=q_summing, normalize=False
        )

    # Reduce the sample data at 30Hz
    r_meas = template.process_from_template_ws(
        meas_ws_30Hz, template_data, q_summing=q_summing, normalize=False
    )
    ...
    if not np.fabs(_binning_60hz - _binning_ref) < 1e-5:
        print(
            "ERROR: The binning of the 60 Hz reference is not the same as the dynamic template"
        )
    ...
    print(
        "60Hz:      %g %g    [%g]"
        % (data_60Hz[0].min(), data_60Hz[0].max(), _binning_60hz)
    )
    ...
    _q_idx_60 = np.asarray(
        np.where(
            (data_60Hz[0] > _min_q * (1 - _tolerance))
            & (data_60Hz[0] < _max_q * (1 + _tolerance))
        )
    )[0]
    _q_idx_meas30 = np.asarray(
        np.where(
            (r_meas[0] > _min_q * (1 - _tolerance))
            & (r_meas[0] < _max_q * (1 + _tolerance))
        )
    )[0]
    _q_idx_ref30 = np.asarray(
        np.where(
            (r_ref[0] > _min_q * (1 - _tolerance))
            & (r_ref[0] < _max_q * (1 + _tolerance))
        )
    )[0]
    ...
    if not data_60Hz[0][_q_idx_60].shape[0] == r_ref[0][_q_idx_ref30].shape[0]:
        ...
        print(
            "Array sizes: %g %g %g"
            % (len(_q_idx_60), len(_q_idx_meas30), len(_q_idx_ref30))
        )
