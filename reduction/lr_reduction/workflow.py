...
def reduce(ws, template_file, output_dir, average_overlap=False, q_summing=False, bck_in_q=False, is_live=False):
    """
    ...
    qz_mid, refl, d_refl, meta_data = template.process_from_template_ws(
        ws, template_file, q_summing=q_summing, tof_weighted=q_summing, clean=q_summing, bck_in_q=bck_in_q, info=True
    )
...
def reduce_fixed_two_theta(
    ws, template_file, output_dir, average_overlap=False, q_summing=False, bck_in_q=False, peak_width=10, offset_from_first=True
):
    """ """
    ...
    if offset_from_first and sequence_number > 1 and os.path.isfile(options_file):
        ...
        options = dict(twotheta_offset=twotheta - 2 * ths_value, pixel_offset=pixel_offset, tthd_db=tthd_db, tthd_sc=tthd_value)
        with open(options_file, "w") as fp:
            json.dump(options, fp)
    print("    Two-theta = %g" % twotheta)

    # Modify the template with the fitted results
    print("    Template peak: [%g %g]" % (template_data.data_peak_range[0], template_data.data_peak_range[1]))
    ...
    print("    New peak:      [%g %g]" % (template_data.data_peak_range[0], template_data.data_peak_range[1]))
    print("    New bck:       [%g %g]" % (template_data.background_roi[0], template_data.background_roi[1]))
