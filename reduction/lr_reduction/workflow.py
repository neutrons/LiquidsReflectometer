...
def reduce(
    ...
    seq_list, run_list = assemble_results(
        meta_data["sequence_id"], output_dir, average_overlap, is_live=is_live
    )
    ...
    with open(template_file, "r") as fd:
        ...
        for i in range(len(seq_list)):
            if len(data_sets) >= seq_list[i]:
                ...
                print(
                    "Too few entries [%s] in template for sequence number %s"
                    % (len(data_sets), seq_list[i])
                )

    # Save the template that was used
    xml_str = reduction_template_reader.to_xml(new_data_sets)
    with open(
        os.path.join(output_dir, "REF_L_%s_auto_template.xml" % run_list[0]), "w"
    ) as fd:
        fd.write(xml_str)


def reduce_fixed_two_theta(
    ws,
    template_file,
    output_dir,
    average_overlap=False,
    ...
    if offset_from_first and sequence_number > 1 and os.path.isfile(options_file):
        ...
        db_center, db_width, _ = peak_finding.fit_signal_flat_bck(
            _x, _y, x_min=x_min, x_max=x_max, center=peak_center
        )
        print(
        ...
        pixel_offset = (
            sc_center
            - (template_data.data_peak_range[1] + template_data.data_peak_range[0])
            / 2.0
        )
        ...
        sample_det_distance = (
            event_reduction.EventReflectivity.DEFAULT_4B_SAMPLE_DET_DISTANCE
        )
        twotheta = (
            np.arctan((db_center - sc_center) * pixel_width / sample_det_distance)
            / 2.0
            * 180
            / np.pi
        )
        ...
        options = dict(
            twotheta_offset=twotheta - 2 * ths_value,
    ...
    print(
        "    Template peak: [%g %g]"
        % (template_data.data_peak_range[0], template_data.data_peak_range[1])
    )

    template_data.data_peak_range = np.rint(
        np.asarray(template_data.data_peak_range) + pixel_offset
    ).astype(int)
    template_data.background_roi = np.rint(
        np.asarray(template_data.background_roi) + pixel_offset
    ).astype(int)
    print(
        "    New peak:      [%g %g]"
        % (template_data.data_peak_range[0], template_data.data_peak_range[1])
    ...
    print(
        "    New bck:       [%g %g]"
        % (template_data.background_roi[0], template_data.background_roi[1])
    )
    ...
    # Assemble partial results into a single R(q)
    seq_list, run_list = assemble_results(
        meta_data["sequence_id"], output_dir, average_overlap
    )
...
def reduce_explorer(
    ws, ws_db, theta_pv=None, center_pixel=145, db_center_pixel=145, peak_width=10
):
    """ """
    from . import peak_finding
    ...
    if theta_pv is None:
        if (
            ws.getRun().getProperty("BL4B:CS:ExpPl:OperatingMode").value[0]
            == "Free Liquid"
        ):
            theta_pv = "thi"
        else:
    ...
    qz, refl, d_refl = event_refl.specular(
        q_summing=False, tof_weighted=False, bck_in_q=False, clean=False, normalize=True
    )
