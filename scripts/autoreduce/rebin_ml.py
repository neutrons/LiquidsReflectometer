...
def smooth_and_rebin(file_path, reference, output_file):
    ...
    ws_final = RebinToWorkspace(WorkspaceToRebin=ws_smoothed, WorkspaceToMatch=ws_ref)

    ws_final = ConvertToPointData(ws_final)

    SaveAscii(InputWorkspace=ws_final, Filename=output_file, WriteXError=False, WriteSpectrumID=False, Separator="Space")
