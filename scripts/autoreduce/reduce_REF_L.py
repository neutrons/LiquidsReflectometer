...
if old_version:
    SetInstrumentParameter(
        ws, ParameterName="dq-constant", Value="0.0", ParameterType="Number"
    )
...
if os.path.isfile(default_file_path):
    ...
    try:
        ...
        os.system(
            "/SNS/software/nses/bin/confirm-data -s Yes BL-4B %s 1 Auto" % ipts_number
        )
    except:  # noqa E722
    ...
    if int(run_number) - sequence_id < 10:
        for i in range(0, 10):
            ...
            reduced_file_name2 = "REFL_%s_%s_%s_partial.txt" % (
                sequence_id,
                _id + 1,
                _run,
            )
