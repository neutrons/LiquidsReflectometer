...
class CompareTwoNXSDataForSFcalculator(object):
    ...
    def __init__(self, nxsdataToCompareWith, nxsdataToPosition):
        ...
        pcharge1 = (
            self.nexusToCompareWithRun.getProperty("gd_prtn_chrg").value
            / nxsdataToCompareWith.getNEvents()
        )
        pcharge2 = (
            self.nexusToPositionRun.getProperty("gd_prtn_chrg").value
            / nxsdataToPosition.getNEvents()
        )
    ...
    def compareParameter(self, param, order, tolerance=0.0):
        ...
        _paramNexusToCompareWith = float(
            _nexusToCompareWithRun.getProperty(param).value[0]
        )
...
class ScalingFactor(object):
    ...
    def execute(self):
        ...
        for run in run_list:
            workspace = LoadEventNexus(
                Filename="REF_L_%s" % run, OutputWorkspace="__data_file_%s" % run
            )
    ...
    def find_peak(self, ws, crop=25, factor=1.0):
        """
        ...
        _data_ws = CreateWorkspace(
            DataX=np.arange(len(signal)), DataY=signal, DataE=np.sqrt(signal)
        )
        ...
        peak = [
            round(peak_center - factor * peak_width),
            round(peak_center + factor * peak_width),
        ]
    ...
    def find_peak_scipy(self, ws):
        ...
        peaks, props = find_peaks(
            counts[_crop:-_crop], threshold=None, width=3, prominence=0.5 * avg
        )
        ...
        try:
            peak = [
                np.int(
                    np.floor(peaks[_peak_index] + _crop - 2.0 * width[0][_peak_index])
                ),
                np.int(
                    np.floor(peaks[_peak_index] + _crop + 2.0 * width[0][_peak_index])
                ),
    ...
    def _compute_scaling_factors(self, lr_data_sorted):
        ...
        for r in lr_data_sorted:
            ...
            if (
                _current_thi is None
                or abs(thi - _current_thi) > THI_TOLERANCE
                or not _current_wl == wl_
            ):
                # New group
                _current_wl = wl_
                _current_thi = thi
        ...
        for g in group_list:
            ...
            tof_min = (
                ...
                / h
                * m
                * (wl + wl_offset * 60.0 / chopper_speed - 1.7 * 60.0 / chopper_speed)
                * 1e-4
            )
            tof_max = (
                source_detector_distance
