...
from lr_reduction import workflow  # noqa E402
...
def reduction():
    ...
    if len(expt) > 0:
        ...
        reduced_data = os.path.join(
            output_dir, "REFL_%s_live_estimate.txt" % first_run_of_set
        )
...
def time_resolved():
    ...
    tof = mtd_api.Rebin(
        ws, [ws.getTofMin(), 300, ws.getTofMax()], OutputWorkspace="tof_"
    )
    x = tof.readX(0)
    x = (x[1:] + x[:-1]) / 2.0
    y = tof.readY(0)
    ...
    if "previous_data" in mtd_api.mtd and len(time_data) > 0:
        ...
        tof_previous_data = mtd_api.Rebin(
            _previous_data,
