...
def reduction():
    ...
    if len(expt) > 0:
        ...
        first_run_of_set = workflow.reduce(
            ws, template_file, output_dir, average_overlap=False, q_summing=False, bck_in_q=False, is_live=True
        )
...
def time_resolved():
    ...
    if "previous_data" in mtd_api.mtd and len(time_data) > 0:
        ...
        tof_previous_data = mtd_api.Rebin(_previous_data, [ws.getTofMin(), 300, ws.getTofMax()], OutputWorkspace="tof_previous_data_")
...
if LIVE_DATA_WS in mtd_api.mtd:
    try:
        ...
        if plotting_ready:
            ...
            publish_plot("REF_L", RUN_NUMBER, files={"file": html_div}, config="/SNS/REF_L/shared/.livedata.conf")
    except:
