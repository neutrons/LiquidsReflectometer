...
for r in range(first_run, last_run + 1):
    _data_file_path = os.path.join(
        "/SNS", "REF_L", ipts, "nexus", "REF_L_%d.nxs.h5" % r
    )
    ...
    if not os.path.isfile(_data_file_path):
        ...
        if new_version:
            cmd = (
                "%s /SNS/REF_L/shared/autoreduce/reduce_REF_L.py %s %s new %s %s %s"
                % (
                    PYTHON_CMD,
                    _data_file_path,
                    _output_dir,
            ...
            if template_file is not None:
                ...
                cmd = "%s /SNS/REF_L/shared/autoreduce/reduce_REF_L.py %s %s" % (
                    PYTHON_CMD,
                    _data_file_path,
                    _output_dir,
                )
...
print(
    ...
    % ((t_1 - t_0), ((t_1 - t_0) / (last_run - first_run + 1)))
)
