...
from lr_reduction.legacy import time_resolved
...
def test_reduce_template_workflow(nexus_dir):
    ...
    with amend_config(data_dir=nexus_dir):
        reduced = time_resolved.reduce_slices(
            ...
            scan_index=5,
            create_plot=False,
        )[0]  # there's only one element in the returned list
    q_values = reduced[0]
    ...
    time_resolved.plot_slices([reduced], "Test", 300, os.path.join(output_dir, "reduced_template.png"), show=False)
